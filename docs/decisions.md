...
**Decision:** Rewrite every `warm_professional` body in the source JSON with three rules: (1) vary the opening so a tenant who reads the library top-to-bottom doesn't see the same first three words five times in a row, (2) sound like a senior Australian tradie running their own business — courteous and confident, not corporate-stiff or apologetic, (3) keep the existing token contract (`{{first_name?, }}`, `{{call_out_fee}}`, etc.) so no validator/seeder changes were needed. Also expanded trade-specific overrides for `warm_professional` from 4 rows to 14 — added trade-tailored emergency `tier_2_infrastructure` guidance for plumber (mains stop tap), electrician (main breaker), HVAC (gas isolation/wall switch); locksmith `after_hours` (well-lit public area for lockouts); and trade-specific `quote_request/needs_inspection` justifications for builder, cleaner, landscaper, pest-control where the "why we need to see it" reasoning genuinely differs by trade.

**Status:** Accepted; applied to live DB (1050 rows updated, 0 inserted/skipped — every standard-tier row had its `version` incremented and body refreshed in place via the `idx_templates_lookup_standard` upsert). All 38 unit tests in `tests/unit/templateProposer.test.js` + `tests/unit/templateSelector.test.js` pass. The 10-row delta vs. the pre-existing DB count of 1060 is an `other/generic_faq/universal_fallback` row that isn't sourced from the JSON catalogue and was left untouched.

**Impl:** `content/templates/templates.v1.json` — 36 base body rewrites under `templates.<intent>.<parent_slug>.warm_professional`; trade overrides under `tradeOverrides.{plumber,electrician,hvac,builder,cleaner,landscaper,pest-control,locksmith}`. Re-seeded via `DATABASE_URL=… node scripts/seed-templates.js`. No schema, worker, or portal code changed.
