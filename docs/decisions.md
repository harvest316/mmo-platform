...
3. **No-match → on-demand Opus proposal, no autosend.** When the runtime selector returns `no_match`, fire a one-off Opus call with (message, current template name list, KB) and ask for a draft template that would fit this conversation. Store as `approval_source='llm_proposed_from_conversation'`, link back to the originating `conversation_id`. No autosend timer — this is explicitly owner-in-the-loop.
...
**Status:** Proposed — supersedes the partial wiring implied in DR-236. DR-236 (templates page + approval endpoints) is the prerequisite; this DR is the runtime rewrite + growth loop.

**Impl (sequenced):**
- **Phase 237a — Runtime cutover (blocking):** rewrite `generateReply`/`buildSystemPrompt` in `workers/index.js`; stop passing KB JSON to the LLM; wire the template selector call; render via `src/services/templateRenderer.js`. Add `no_match` handling with the on-demand Opus draft path (#3). Add feature flag per-tenant so we can dogfood before cutting all tenants over.
