...
3. **No-match handling is tradie-configurable** (`tenant.settings.no_match_behavior`, three values). When the runtime selector returns `no_match`, the response path branches on this setting:
   - **`llm_freeform` (default)** — runs a single free-form LLM reply *for this message only* (the existing `generateReply` path), and in parallel fires an Opus call to propose a template that would fit this conversation. Customer never waits, library grows from real gaps. The free-form fallback is acceptable transitional state for new tenants whose libraries are thin — once the proposal is approved, the next inbound on the same intent goes templated.
   - **`fallback_template`** — autosends a single universal fallback template (`"Thanks for your message — we'll get back to you shortly."` or tenant-edited equivalent), AND fires the Opus proposal in parallel. Stricter than default — guarantees zero free-form output even on no_match.
...
**Impl (sequenced):**
- **Phase 237a — Runtime cutover (blocking):** migration 018 for `crai.template_proposals` table + the `no_match_behavior` enum check on `tenant.settings`. New services: `templateSelector.js` (Haiku call, structured output), `templateProposer.js` (Opus draft on no_match), `requiredSlots.js` (parse body for required tokens). Universal fallback template seeded one-per-trade. Rewrite `generateReply` to: select → render → send, branching on `no_match_behavior`. Settings UI exposes the no_match enum dropdown + the kill-switch checkbox.
