...
3. **No-match handling is tradie-configurable** (`tenant.settings.no_match_behavior`, three values). When the runtime selector returns `no_match`, the response path branches on this setting:
   ...
   - **`llm_freeform` (opt-in only, never default)** — runs a single free-form LLM reply *for this message only* (the legacy `generateReply` path), and in parallel fires an Opus call to propose a template. Customer never waits, library grows from real gaps. **Note:** this setting sends AI-authored outbound content, which (a) voids the ACL s18 "approved replies do the talking" marketing claim for these specific messages, (b) may trigger PI insurance AI-exclusion endorsements (ISO CG 40 47 / Berkley PC 51380) more aggressively, (c) falls outside the DR-224 Phase 2.3 architectural commitment. UI shows an explicit disclosure when the tenant selects this option.

   In all three modes the proposal lands in `crai.template_proposals` (linked to `conversation_id`) and surfaces in the "New reply pattern" chip on `/conversations`. Setting lives at `tenant.settings.no_match_behavior`; UI exposed via the existing `/settings` page.

   **Default rationale (revised 2026-04-23 after re-reading `docs/governance/architecture-commitment.md`):** the initial draft of this DR had `llm_freeform` as default on the logic that thin-library tenants would otherwise get a generic holding reply. On review that directly violates three truths we've shipped in sales copy and ToS — "AI reads, templates reply", "won't invent prices or services", "your approved replies do the talking". Defaulting to free-form would silently undo those claims. `fallback_template` is the only default that keeps the architectural commitment intact; tenants who consciously trade the commitment for live AI replies can opt in via the settings UI with full disclosure.
...
- **Phase 237b — Authoring-time generator:** `src/services/templateProposer.js` (Opus call, KB section + library as input); trigger hook on KB save (`PUT /api/knowledge-base`) diffs prior vs current and enqueues proposals only for sections with meaningful changes (services/FAQ/policies/customNotes). New `approval_source` enum values.
